...
public actor PodsService {
    ...
    /// Take away every pod a container was given that no container is in.
    ///
    /// A pod nobody named goes as its container is removed, but a container can
    /// go without that running: one run to be removed on exit that was still
    /// running when the control plane stopped is reaped from disk as the
    /// containers service loads, before this service is there to be asked. So
    /// once both services are up, each such pod still holding no container is
    /// taken away the way a removal takes it. Nothing can place a container in
    /// one meanwhile, since this runs before the server listens.
    ///
    /// The kubelet's garbage collection removes a sandbox that holds no
    /// containers the same way, stopping it first. It keeps one that is still
    /// ready, since the pod's next container may be headed there; nothing is
    /// headed for a pod that was made for a container that is gone.
    /// https://github.com/kubernetes/kubernetes/blob/master/pkg/kubelet/kuberuntime/kuberuntime_gc.go
    public func removeEmptyAnonymousPods() async {
        for id in self.pods.keys.sorted() {
            await self.removeIfAnonymous(id: id)
        }
    }
